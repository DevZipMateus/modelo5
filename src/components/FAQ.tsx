...
const FAQ = () => {
  ...
  return <section id="faq" ref={sectionRef} className="py-20 bg-transparent">
      <div className="section-container">
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {faqs.map((faq, index) => <Collapsible key={index} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 opacity-0" ref={el => elementsRef.current[2 + index] = el}>
              <CollapsibleTrigger className="w-full p-6 text-left flex items-center justify-between hover:bg-tech-lightgray/50 rounded-xl transition-colors duration-300 group">
                <h3 className="text-tech-blue font-semibold text-lg pr-4 group-hover:text-tech-orange transition-colors duration-300">
                  {faq.question}
                </h3>
                <ChevronDown className="h-5 w-5 text-tech-gray group-hover:text-tech-orange transition-all duration-300 group-data-[state=open]:rotate-180" />
              </CollapsibleTrigger>
        ...
        <div className="text-center mt-12">
          <div className="bg-white rounded-2xl p-8 shadow-lg max-w-2xl mx-auto opacity-0" ref={el => elementsRef.current[10] = el}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="https://wa.me/5555999887766?text=Olá!%20Tenho%20algumas%20dúvidas%20sobre%20os%20serviços." target="_blank" rel="noopener noreferrer" className="bg-tech-green hover:bg-tech-green/90 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105">
                Falar no WhatsApp
              </a>
              <button className="bg-tech-blue hover:bg-tech-darkblue text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300" onClick={() => {
